function resolveMutationError(resultOrError, fallbackErrorMessage) {
...
export async function runMutation({
  ...
  try {
    ...
    if (result?.error) {
      const error = resolveMutationError(result, fallbackErrorMessage);
    ...
    const error = resolveMutationError(thrownError, fallbackErrorMessage);
    if (typeof onError === "function") {
      onError(error);
    }
    if (typeof setFormError === "function") {

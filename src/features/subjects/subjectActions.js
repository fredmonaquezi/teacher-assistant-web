...
export default function createClassSubjectActions({ refreshSubjectData, setFormError }) {
  ...
  const refresh = async () => {
    if (await refreshSubjectData() === false) throw new Error("Subjects saved, but could not be reloaded. Please retry.");
  };
